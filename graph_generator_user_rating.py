...
class Neo4jHandler:
                    ...
                    if "book" in file_name:
                        ...
                        for _, row in ratings_df.iterrows():
                            ...
                            session.write_transaction(
                                self._create_review_relation,
                                row['Name'],
                                user_id,
                                row['Rating']  # Include the rating column
                            )
            except Exception as e:
    ...
    @staticmethod
    def _create_review_relation(tx, book_name, user_id, rating):
        query = (
            "MATCH (b:Book {Name: $book_name}) "
            "MATCH (u:User {ID: $user_id}) "
            "MERGE (u)-[r:REVIEWED_BY]->(b) "
            "ON CREATE SET r.rating = $rating "
            "ON MATCH SET r.rating = $rating"  # Update rating if the relationship already exists
        )
        try:
            tx.run(query, book_name=book_name, user_id=user_id, rating=rating)
        except Exception as e:
            logging.error(f"Failed to create review relation with rating: {e}")
            raise
